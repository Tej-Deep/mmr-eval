...
# Run this script from the qwen_training directory: bash scripts/relaunch_single_sft_32b_job.sh
#
# Usage:
#   bash scripts/relaunch_single_sft_32b_job.sh MODEL_PATH DATASET_NAME
#
# Example to re-launch the failed job you mentioned:
#   bash scripts/relaunch_single_sft_32b_job.sh "./custom_token_models/Qwen2.5-VL-32B-Instruct-updated-tokens" "visualprm_data_no_perception_v1%100"

# Check arguments
if [ $# -ne 2 ]; then
    echo "Error: This script requires exactly 2 arguments"
    echo "Usage: bash scripts/relaunch_single_sft_32b_job.sh MODEL_PATH DATASET_NAME"
    echo ""
    echo "Available models from sweep:"
    echo "  - Qwen/Qwen2.5-VL-32B-Instruct"
    echo "  - ./custom_token_models/Qwen2.5-VL-32B-Instruct-updated-tokens"
    echo "  - ./custom_token_models/Qwen2.5-VL-32B-Instruct-updated-tokens-random-init-vals"
    echo ""
    echo "Available datasets from sweep:"
    echo "  - mc0_visualprm_data_errors_no_perception_v2%100"
    echo "  - mc0_visualprm_data_errors_balanced_v2%100"
    echo "  - mc0_visualprm_data_full_non_balanced_v0%100"
    echo ""
    echo "Example:"
    echo "  bash scripts/relaunch_single_sft_32b_job.sh \"./custom_token_models/Qwen2.5-VL-32B-Instruct-updated-tokens\" \"mc0_visualprm_data_errors_no_perception_v2%100\""
    exit 1
fi
